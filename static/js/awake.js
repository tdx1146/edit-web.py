...
function removeOptimisticMessage() {
  // 乐观消息在 store.pairs[0]，user.userIndex === -1 特征
  const firstPair = store.pairs[0];
  if (firstPair && firstPair.user && firstPair.user.userIndex === -1) {
    store.pairs.shift();
...
function resendLastMessage() {
  ...
  const unsent = caches.filter(function(c) { return !c.sent; });
  if (unsent.length > 0) {
    const latest = unsent[unsent.length - 1];
    document.getElementById('awake-editor').value = latest.text;
    // 标记原条目已发，避免重复
    latest.sent = true;
  ...
  if (truncate) {
    ...
    try {
      ...
      if (store.pairs && store.pairs.length > 0 && store.currentPage >= 0 && store.currentPage < store.pairs.length) {
        ...
        // 🌫️ 换算全局用户消息序号（防回滚一页误截断几乎全部对话）
        const globalIndex = await window.userIndexCtl.computeGlobalUserIndexForPair(targetPair);
        const ed = await api.edit(globalIndex, '…', true);
        if (!ed.ok) {
