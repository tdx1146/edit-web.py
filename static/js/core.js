...
const store = {
  ...
  // 🌫️ 会话元信息（含 info.sessionFile，供截断索引换算 user-index.js 使用）
  sessionInfo: null,
};
// 全局别名
['msgCache','pairs','currentPage','totalPages','sessionInfo']
...
async function refresh() {
  try {
    ...
    if (d) {
      // 🌫️ 缓存会话信息（含 sessionFile），供截断索引换算 user-index.js 使用
      if (d.info) store.sessionInfo = d.info;
    }
    if (d && d.pairs) {
