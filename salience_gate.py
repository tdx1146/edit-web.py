...
判据（高阈值 + 连续计数 + 边沿触发，复用 self_pulse v2 模式）：
  ...
  2. entropy_ratio > 0.9 连续 ≥3 次（= LMS 自身 entropy_high_threshold）
  3. purpose_coherence < 0.8 连续 ≥3 次
  4. 事件类型权重：anomaly=0.9 > dream/相变=0.6 > task_complete=0.3 > 例行=0.1
  5. 合流规则：score = 0.35*novelty + 0.35*salience + 0.30*goal，
     单一高不唤醒，总分 > 0.6 才放行。anomaly（0.9）也需至少一路 LMS
     确认合流（self_pulse 漂移告警在第 3 连击脉冲时天然满足：salience
     0.35 + goal 0.27 = 0.62 > 0.6）；SG_ANOMALY_BYPASS=1 可强制直通
     （默认关，宁可漏报不可误报）。
...
ENTROPY_HIGH = float(os.environ.get('SG_ENTROPY_HIGH', '0.9'))  # 同 self_pulse
