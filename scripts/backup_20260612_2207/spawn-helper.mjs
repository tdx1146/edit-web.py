...
const method = methodArg || 'send'; // "send", "abort", or "spawn"
if (!sessionKey) {
  process.stderr.write('Usage: inject-helper.mjs <sessionKey> <message> [send|abort]\n');
  process.exit(1);
}
if (method !== 'abort' && !message) {
...
// Device auth: check if dangerouslyDisableDeviceAuth is true
const dangerouslyDisableDeviceAuth = config &&
  getConfig(config.data, 'gateway.controlUi.dangerouslyDisableDeviceAuth') === true;
...
async function recvFrame(timeoutMs = 10000) {
...
async function main() {
  ...
  if (method === 'abort') {
    ...
    sendFrame(JSON.stringify({
      type: 'req', id: 'spawn-' + crypto.randomUUID().slice(0, 8),
      method: 'agent.spawn',
      params: { task: message, model: 'GLM-Z1-Flash', mode: 'run', timeout: 60 },
    }));
  } else {
    sendFrame(JSON.stringify({
  ...
  // Wait briefly for the send acknowledgment
  try {
    const ack = await recvFrame(5000);
    if (ack.type === 'text') {
      const r = JSON.parse(ack.data);
      if (!r.ok) throw new Error('Send failed: ' + JSON.stringify(r.error));
    }
  } catch (e) {
    // Timeout is OK — message was sent, just response hasn't come yet
  }
