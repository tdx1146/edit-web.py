...
#         elif clean_path == '/api/encrypt-save-file':
    ...
    def _get_cache_stats(self):
        """解析会话文件，返回上轮缓存命中统计（只读文件末尾的最近数据）"""
        try:
            import glob, os
            files = [f for f in glob.glob(os.path.join(DATA_DIR, '*.jsonl')) if '.trajectory' not in f]
            if not files:
                return {"rounds": [], "summary": {}}
            sf = max(files, key=os.path.getctime)
            rounds = []
            with open(sf, 'rb') as f:
                # 从文件末尾倒读，最多取 50 条 assistant
                f.seek(0, 2)  # 结尾
                pos = f.tell()
                buf = b''
                chunk = 4096
                while pos > 0 and len(rounds) < 50:
                    ...
                    for line in reversed(lines):
                        ...
                        rounds.append({
                            ...
                            "cachePct": rate,
                            "cost": round((cache * 0.025 + inp * 3.0 + out * 8.0) / 1000000, 5),
                        })
            # 只返回最近一条（上轮缓存）
            last = rounds[0] if rounds else {}
            return {"ok": True, "rounds": [last] if last else [], "summary": last}
        except Exception as e:
            return {"ok": False, "error": str(e)}
