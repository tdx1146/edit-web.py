...
BROWSE_ROOT = os.environ.get('BROWSE_ROOT') or os.path.dirname(ALL_AUTO_DIR)  # /vol2/1000/AI专用
...
def read_session(session_file, max_messages=200):
    ...
    对于大文件（>5000行）只取末尾 max_messages 条，避免加载耗时过长。
    前端翻页走 RPC 接口获取完整数据。
    """
    ...
    try:
        ...
        tail_n = max_messages * 3  # 多读一些，过滤后取到需要的数量
    except:
        need_tail = False
        tail_n = 0
            ...
            with open(session_file, 'rb') as f:
                ...
                while pos > 0 and len(lines) < tail_n:
                    ...
                    if pos > 0:
                        ...
                        buf = b''
                    lines = [p.decode('utf-8', errors='replace').strip() for p in parts if p.strip()] + lines
...
def _digestion_skill_status():
    ...
    # 📦 skill 数量
    import glob
    result["skill_count"] = len(glob.glob(os.path.expanduser("~/.pi/agent/skills/*/SKILL.md")))
...
def _digestion_history():
    ...
    entries = []
    try:
        with open(CRON_RUNS, encoding='utf-8') as f:
            for line in f:
                line = line.strip()
...
def _system_health():
    ...
    result = {
        ...
        "cron": {"enabled": False, "last_ok": None},
        "context": {"expected": 1000000, "actual": 0, "ok": False}
    }
    ...
    try:
        ...
        ds_models = cfg.get("models", {}).get("providers", {}).get("DeepSeek", {}).get("models", [])
        for m in ds_models:
            if m.get("id") == "deepseek-v4-flash":
                result["context"]["actual"] = m.get("contextWindow", 0)
                result["context"]["ok"] = m.get("contextWindow", 0) == 1000000
    except:
        pass
    
    cron_path = "/vol1/@apphome/trim.openclaw/data/home/.openclaw/cron/jobs.json"
    try:
        with open(cron_path) as f:
            cron_cfg = _json.load(f)
        for job in cron_cfg.get("jobs", []):
            if "武器库" in job.get("name", "") or "weapon" in job.get("name", "").lower():
                result["cron"]["enabled"] = job.get("enabled", False)
                result["cron"]["last_ok"] = job.get("state", {}).get("lastRunStatus")
                break
    except:
        pass
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif clean_path == '/api/cache-stats':
            self._send_json(200, self._get_cache_stats())
        elif clean_path == '/api/subagents':
            self._send_json(200, self._list_subagents())
        elif clean_path == '/api/exec-subagent':
            self._handle_exec_subagent()
        elif clean_path == '/api/subagent-history':
            self._send_json(200, {"ok": True, "entries": _get_subagent_history(30)})
        elif clean_path == '/api/lungan-status':
            self._send_json(200, _lungan_status())
        elif clean_path == '/api/backup-stale':
            self._send_json(200, _backup_stale_status())
        elif clean_path == '/api/system-health':
            self._send_json(200, _system_health())
        elif clean_path == '/api/weaponry-toggle':
            self._send_json(200, _weaponry_toggle_status())
        elif clean_path == '/api/thinking-status':
            self._send_json(200, _thinking_status())
        elif clean_path == '/api/plugin-health':
            self._send_json(200, _plugin_health())
        elif clean_path == '/api/last-injection':
            self._send_json(200, _last_injection())
        elif clean_path == '/api/last-processing':
            self._send_json(200, _last_processing())
        elif clean_path == '/api/digestion-status':
            self._send_json(200, _digestion_status())
        elif clean_path == '/api/digestion-skill':
            self._send_json(200, _digestion_skill_status())
        elif clean_path == '/api/digestion-history':
            self._send_json(200, _digestion_history())
        elif clean_path == '/api/backlog':
            self._send_json(200, _backlog_status())
        elif clean_path == '/api/reminders':
            self._handle_reminders()
        elif clean_path == '/api/secretary-log':
            self._handle_secretary_log()
        elif clean_path == '/api/facts-stale':
            self._handle_facts_stale_check()
        elif clean_path == '/api/session-rpc':
            self._handle_session_rpc()
        elif self.path.startswith('/api/session'):
            self._send_json(200, self._get_session_data())
        elif clean_path == '/api/backups':
            self._send_json(200, self._list_backups())
        elif clean_path == '/api/quickcheck':
            self._send_json(200, self._handle_quickcheck())
#         elif clean_path == '/api/awake-questions/random':
#             self._handle_awake_questions()
        elif clean_path == '/api/awake-questions/list':
            self._handle_awake_list()
        elif clean_path == '/api/awake-questions/save':
            self._handle_awake_save()
#         elif clean_path == '/api/awake-questions/send':
#             self._handle_awake_send()
        ...
        elif clean_path == '/api/memory-files':
...
#             self._handle_encrypt_folders()
            ...
            return
        elif clean_path == '/paper-annotated.html':
            paper_candidates = [
            ...
            _page_bytes = _get_html_page().encode()
            _accept_enc = self.headers.get('Accept-Encoding', '')
            _use_gzip = 'gzip' in _accept_enc and len(_page_bytes) > 1024
            if _use_gzip:
                _page_bytes = gzip.compress(_page_bytes)
            ...
            fpath = os.path.join(_THIS_DIR, clean_path.lstrip('/'))
            if os.path.exists(fpath):
                ...
                self._serve_static_file(fpath, mime)
            else:
...
#         elif clean_path == '/api/awake-questions/send':
#             self._handle_awake_send()
        elif clean_path == '/api/abort':
            self._handle_abort()
        elif clean_path == '/api/restart-http':
            self._handle_restart_http()
        elif clean_path == '/api/weaponry-toggle':
            self._handle_weaponry_toggle()
        elif clean_path == '/api/pet-me':
            self._handle_pet_me()
        elif clean_path == '/api/thinking-toggle':
            self._handle_thinking_toggle()
        elif clean_path == '/api/spawn-subagent':
            self._handle_spawn_subagent()
        elif clean_path == '/api/auth-subagent':
            self._handle_auth_subagent()
        elif clean_path == '/api/exec-subagent':
            self._handle_exec_subagent()
        elif clean_path == '/api/reminders':
            self._handle_reminders()
        elif clean_path == '/api/trim-session':
            self._handle_trim_session()
        elif clean_path == '/api/memory-file':
            self._handle_memory_file_save()
#         elif clean_path == '/api/encrypt-save-file':
        ...
        elif clean_path == '/api/pass-password':
    ...
    _session_cache = None
    _session_cache_ts = 0
    _SESSION_CACHE_TTL = 1.5  # 秒

    def _invalidate_session_cache(self):
        """写入操作后调用，使缓存失效"""
        self.__class__._session_cache = None
        self.__class__._session_cache_ts = 0

    def _get_session_data(self):
        import time as _time
        now = _time.time()
        cache = self.__class__._session_cache
        if cache and (now - self.__class__._session_cache_ts) < self.__class__._SESSION_CACHE_TTL:
            return cache

        sk, session_file = get_session_info()
        if not sk and not session_file:
            result = {"error": "no session", "messages": [], "pairs": [], "info": {
                "host": "127.0.0.1", "port": GATEWAY_PORT, "sessionFile": None,
                "dataDir": DATA_DIR,
            }}
            self.__class__._session_cache = result
            self.__class__._session_cache_ts = now
            return result
    ...
    def _get_cache_stats(self):
        """解析会话文件，返回缓存命中统计（已拆分到 cache_stats_helper.py）"""
    ...
    def _handle_list_files(self):
        ...
        try:
            ...
            if not os.path.isdir(folder_path):
                ...
                return
            files = sorted([f for f in os.listdir(folder_path) if f.endswith(('.md', '.docx', '.txt'))])
            file_info = []
            for f in files:
    ...
    def _handle_tb_save_file(self):
        try:
            ...
            # 完整 diff 追踪
            try:
                is_novel = _is_novel_path(path)
                _log_file_save(path, content, is_novel, old_content=old_content_before)
            except Exception:
                pass
            # 🔍 小秘书静默分析：用户保存 .md 文件时，spawn 子代理分析变更
            try:
                _secretary_analyze_save(path, content, old_content_before)
            except Exception:
                pass
        ...
        try:
            ...
            if path.endswith('.docx'):
                try:
                    ...
                    self._send_json(200, {"ok": True, "content": f"[docx 读取失败: {e}]"})
                    return
            # 直接读取
            try:
                with open(path, 'r', encoding='utf-8') as f:
