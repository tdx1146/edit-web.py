...
def get_cache_stats(data_dir):
    ...
    """
    try:
        # 优先从 sessions.json 取主会话文件，避免 glob 所有文件
        sf = None
        sessions_json = os.path.join(data_dir, 'sessions.json')
        if os.path.exists(sessions_json):
            try:
                with open(sessions_json) as _f:
                    store = json.load(_f)
                for _k in ('agent:main:main',):
                    _entry = store.get(_k)
                    if _entry:
                        _sf = _entry.get('sessionFile', '')
                        if _sf and os.path.exists(_sf):
                            sf = _sf
                            break
                if not sf:
                    # fallback: 找任意第一个有 sessionFile 的
                    for _k, _v in store.items():
                        _sf = _v.get('sessionFile', '')
                        if _sf and os.path.exists(_sf):
                            sf = _sf
                            break
            except Exception:
        ...
        # 上一轮 = 寻找最近一个有缓存数据的轮次
        # rounds[0] 是当前正在生成的回复（cachePct=0），跳过它
        prev = None
        for r in all_rounds[1:]:
            if r.get('cacheRead', 0) > 0 and r.get('cachePct', 0) > 0:
                prev = r
                break
        if prev is None and len(all_rounds) > 1:
            # 仅当所有历史轮都0缓存时，回退到第二新
            prev = all_rounds[1]
        elif prev is None and len(all_rounds) == 1:
            # 只有一轮（新session或刚压缩），显示暂无
            prev = None
